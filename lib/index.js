 ...
 * convert to Burmese
 */
export const keepBurmese = str => str.replace(/[0-9]/g,i=>digitBurmese[parseInt(i)]);

/**
 * @param {string} str
 * convert from Burmese
 */
export const turnBurmese = str => str.split('').map(i => digitBurmese.includes(i)?digitBurmese.indexOf(i):i).join('');
...
function sense(n='0'){
   ...
   */
  var raw = [];
  // var row = { number: keepBurmese(n), digit: n, sense:''};
  var row = { sense:'' };
...
export function get(str) {
  if (typeof str == 'string'){
    if (new RegExp(digitBurmese.join("|")).test(str) == true) str = turnBurmese(str);
   ...
   */
  var result ={number:keepBurmese(query),digit:query,notation:[]};
...
/** @namespace */
const myanmarNotation = {keepBurmese,turnBurmese,multiplication,get};
